...
%        for idx=1:length(gsd)
            ...
            fdx  = isfinite(bcc(bdx).val(:));
            vali = interp1(time_m(fdx),bcc(bdx).val(fdx),time_mi,'pchip',NaN);
            ndx = vali < 0;
            vali(ndx) = 0;
            ...
            vali(fdx2)    = interp1(time_m(fdx),bcc(bdx).val(fdx),time_mi(fdx2),'nearest','extrap');
            ...
            fprintf(fid,'parameter            ''Salinity             end A uniform''       unit ''[ppt]''\n');
            fprintf(fid,'parameter            ''Salinity             end B uniform''       unit ''[ppt]''\n');
